...
public class TransformServlet extends SlingAllMethodsServlet {
    ...
    protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)
        ...
        response.setHeader(
                "Content-Disposition", "filename=" + request.getResource().getName());

        log.debug(
                "Transforming resource: {} with transformation: {} to {}",
                request.getResource(),
                transformationName,
                format);
        try {
            ...
            // Check if we're already processing the error resource to prevent recursion
            String errorResourcePath = thumbnailSupport.getServletErrorResourcePath();
            String resourcePath =
            ...
            if (resourcePath != null
                ...
                log.warn("Error occurred while processing error resource, returning empty response to avoid recursion");
                return;
            }
            ...
            RequestDispatcher disp = request.getRequestDispatcher(errorResourcePath, op);
    ...
    private ByteArrayOutputStream transform(
            SlingHttpServletRequest request, SlingHttpServletResponse response, Transformation transformation)
            throws IOException {
