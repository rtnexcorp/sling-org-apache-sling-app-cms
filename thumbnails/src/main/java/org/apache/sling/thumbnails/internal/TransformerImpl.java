...
public class TransformerImpl implements Transformer {
    ...
    public TransformerImpl(
            @Reference(
                    ...
                    List<ThumbnailProvider> thumbnailProviders,
    ...
    private ThumbnailProvider getThumbnailProvider(Resource resource) throws IOException {
        ...
        log.debug(
                "Finding thumbnail provider for resource {} with meta type {} from available providers {}",
                resource,
                metaType,
                thumbnailProviders);
        return thumbnailProviders.stream()
                .filter(tp -> {
    ...
    public TransformationHandler getTransformationHandler(String resourceType) {
        return handlers.stream()
                .filter(h -> resourceType.equals(h.getResourceType()))
                .findFirst()
                .orElse(null);
    }
