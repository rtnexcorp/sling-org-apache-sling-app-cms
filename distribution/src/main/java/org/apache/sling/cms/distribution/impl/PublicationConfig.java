...
public @interface PublicationConfig {
    ...
    @AttributeDefinition(
            name = "%publication.param.type.name",
            description = "%publication.param.type.description",
            options = {
                @Option(label = "Author", value = "AUTHOR"),
                @Option(label = "Renderer", value = "RENDERER"),
                @Option(label = "Standalone", value = "STANDALONE")
            })
    INSTANCE_TYPE instanceType() default INSTANCE_TYPE.STANDALONE;

    @AttributeDefinition(
            name = "%publication.param.mode.name",
            description = "%publication.param.mode.description",
            options = {
