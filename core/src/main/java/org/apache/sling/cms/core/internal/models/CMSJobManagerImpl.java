...
public class CMSJobManagerImpl implements CMSJobManager {
    ...
    public Collection<Job> getJobs() {
        ...
        return Stream.concat(
                        jobManager.findJobs(QueryType.ALL, null, 0, search).stream(),
                        jobManager.findJobs(QueryType.HISTORY, null, 0, search).stream())
                .collect(Collectors.toList());
    }

    private String getJobTitleKey(String jobTopic) {
        return cmsJobManager.getJobs().stream()
                .filter(j -> jobTopic.equals(j.getTopic()))
                .map(ConfigurableJobExecutor::getTitleKey)
                .findFirst()
                .orElse(null);
    }
