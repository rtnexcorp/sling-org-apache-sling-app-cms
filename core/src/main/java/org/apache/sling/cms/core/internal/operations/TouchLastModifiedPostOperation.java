...
@Component(
        ...
        property = Constants.SERVICE_RANKING + "=-1")
public class TouchLastModifiedPostOperation implements SlingPostProcessor {
    ...
    public void process(SlingHttpServletRequest request, final List<Modification> changes) throws Exception {
        ...
        List<String> paths = changes.stream()
                .filter(m -> (m.getType() == ModificationType.CREATE
                        || m.getType() == ModificationType.MODIFY
        ...
        paths.addAll(changes.stream()
                ...
                .map(Modification::getDestination)
                .collect(Collectors.toList()));
        ...
        List<Resource> resources = paths.stream()
                .map(p -> request.getResourceResolver().getResource(p))
                .map(CMSUtils::findPublishableParent)
                .filter(p -> {
        ...
        resources.forEach(r -> {
            ...
            op.ifPresent(mvm -> {
                ...
                mvm.put(
                        CMSConstants.PN_LAST_MODIFIED_BY,
