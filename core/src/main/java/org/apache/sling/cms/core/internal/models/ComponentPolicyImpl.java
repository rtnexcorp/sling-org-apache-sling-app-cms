...
public class ComponentPolicyImpl implements ComponentPolicy {
    ...
    public boolean applies(Resource resource) {
        return Optional.ofNullable(new PageManagerImpl(resource).getPage())
                .map(p -> StringUtils.removeStart(resource.getPath(), p.getPath()))
                .map(p -> p.matches(pathPattern))
                .orElse(false);
    }

    @Override
    public String[] getAvailableComponentTypes() {
        return getPolicyResource()
                .map(pr -> pr.getValueMap().get("availableComponentTypes", new String[0]))
                .orElse(new String[0]);
    }
    ...
    public Map<String, Resource> getComponentConfigs() {
        ...
        Resource container = getPolicyResource()
                .map(pr -> pr.getChild("componentConfigurations"))
                .orElse(null);
        if (container != null) {
