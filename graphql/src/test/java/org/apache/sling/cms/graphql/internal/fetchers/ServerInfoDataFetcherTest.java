...
class ServerInfoDataFetcherTest {
    ...
    @Test
    void testGetReturnsMapWithExpectedKeys() throws Exception {
        ...
        verify(securityService).validateAccess(resolver, "serverInfo");
    }

    @Test
    void testTimestampIsStringAndNotBlank() throws Exception {
        Map<String, Object> result = fetcher.get(environment);

        Object timestamp = result.get("timestamp");
        assertNotNull(timestamp);
        assertInstanceOf(String.class, timestamp);
        assertFalse(((String) timestamp).isBlank());
    }
