    ...
    <c:choose>
        ...
        <c:when test="${sling:getRelativeResource(resource,'options') != null}">
            <c:forEach var="option" items="${sling:listChildren(sling:getRelativeResource(resource,'options'))}">
                <c:set var="checked" value="${value eq option.valueMap.value ? 'checked=\"checked\"' : ''}" />
                <label class="radio">
                    <input type="radio" name="${fieldName_value}" value="${sling:encode(option.valueMap.value,'HTML_ATTR')}" id="${fieldId}_${fn:replace(option.valueMap.value,' ','_')}" ${required} ${disabled} ${checked} />
