...
public class SelectionHandler implements FieldHandler {
    ...
    @ObjectClassDefinition(
            name = "%cms.reference.selection.name",
            description = "%cms.reference.selection.description",
    ...
    private String[] stripBlank(String[] parameterValues) {
        return Optional.ofNullable(parameterValues)
                .map(v -> {
                    List<String> values =
                            Arrays.stream(v).filter(StringUtils::isNotBlank).collect(Collectors.toList());
